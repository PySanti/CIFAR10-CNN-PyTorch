...
class InceptionBlock(nn.Module):
    def __init__(self,  in_channels, out_1x1, red_3x3, out_3x3, red_5x5, out_5x5, pool_proj, db_prob):
        super(InceptionBlock, self).__init__()
        self.db_prob = db_prob
    ...
    def forward(self, X):
        ...
        if concat.size(-1) >= 2 and self.db_prob:
            concat = DropBlock2D(self.db_prob, concat.size(-1)//2)(concat)
        return concat
